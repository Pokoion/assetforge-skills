...
Also add `.env`, `.aforge-debug/`, and `.aforge-*.json` to `.gitignore`. The `.aforge-*.json` entry is a safety net — the CLI auto-deletes temp skill files after it reads them, but ignoring the pattern means any leftover (e.g. from a crashed run) is not committed by accident.
